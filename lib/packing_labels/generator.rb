      ...
      def generate(orders:,request:,product_labels_only:,product_label_format:,print_multiple_labels_per_item:)
        ...
        if product_label_format > 1
          ...
          pages.each do |page|
            ...
            product_font_size = (36 - (page[:a][:data][:product][:product_name].length/5.5))*2.round(0)
            order_num_font_size = (25 - (page[:a][:data][:order][:order_number].length/25))*2.round(0)
