...
describe "MakeInvoicePdfTempFile interactor" do
  ...
  let(:html) { render_html_view(order) }
  ...
  it "builds a document using PDFKit" do
    ...
    context = MakeInvoicePdfTempFile.perform(order: order)
  ...
  it "requires an Order" do
    expect { MakeInvoicePdfTempFile.perform() }.to raise_error(/:order/)
  end


  def render_html_view(order)
