	...
	switch(type){
		...
		case 'dd':
			...
			if(core.catalog.filters.dd != 0){
				...
				$('#filter_dd_'+core.catalog.filters.dd+',#filter_ddpopup_'+core.catalog.filters.dd).parent().removeClass('active');
			}
			...
			if(core.catalog.filters.dd != 0){
				...
				core.catalog.resetDeliveryDateChange();
				$('#filter_dd_'+core.catalog.filters.dd+',#filter_ddpopup_'+core.catalog.filters.dd).parent().addClass('active');
		...
		case 'cartOnly':
			...
			$('#cartFilterCheck').prop('checked',(core.catalog.filters.cartOnly == 1));
			break;
	}
...
core.catalog.sendQty=function(prodId,newQty,dd){
	...
	core.log('dd_id passed: '+dd);
	dd = parseInt(dd);
	if(isNaN(dd) || dd==0)
		dd = parseInt($('#prodDd_' + prodId).val());
	core.log('dd_id after step 1: '+dd);
	if(isNaN(dd) || dd==0)
...
core.catalog.sendNewQtys=function(){
	var items=[];
	var data = '';
	//alert('about to send qts');
	//core.alertHash(core.cart.items);
	for (var i = 0; i < core.cart.items.length; i++){
