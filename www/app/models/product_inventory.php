...
class core_model_product_inventory extends core_model_base_product_inventory
{
	...
	function reduce_inventory($item,$qty=null)
	{
		global $core;
		
		$qty = floatval((is_null($qty))?$item['qty_ordered']:$qty);
		
		
		if($qty != 0)
		{
			core::log('need to adjust '.$item['product_name'].' by '.$qty);
		...
		{
			core::log('no need to adjust '.$item['product_name'].', qty not changed');
		}
	}
}
?>