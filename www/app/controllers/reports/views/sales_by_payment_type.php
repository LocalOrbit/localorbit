...
$items = new core_datatable('sales_by_payment_type','reports/sales_by_payment_type',$col);
# this does the totaling 
function sbpt_formatter($data)
{
	$data['row_discount'] = $data['row_adjusted_total'] - $data['row_total'];

	$data = core_controller_reports::master_formatter('sbpt_',$data);
...
function sbpt_output($format,$dt)
{
	core_controller_reports::master_output_formatter('sbpt_',$format,$dt);
}
...
$items->add_filter(new core_datatable_filter('sbptcreatedat1','lo_fulfillment_order.order_date','>','date',core_format::date($start,'db')));
$items->add_filter(new core_datatable_filter('sbptcreatedat2','lo_fulfillment_order.order_date','<','date',core_format::date($end,'db')));
...
{	
	...
	$items->filter_html .= core_datatable_filter::make_select(
		'sales_by_payment_type',
		'lo_order.domain_id',
		$orders->filter_states['sales_by_payment_type__filter__lo_order_domain_id'],
		$hubs,
		'domain_id',
		'name',
...
$items->add(new core_datacolumn('payment_method','Payment Type',false,'18%','{payment_method}'));
