 ...
 * Umapper
 */
require_once 'Umapper.php';

/**
 * Sorry ugly hacks - messages are used by RPC calls, so we have 
 * to recreate (to a certain extent) global WP vars
 */
...
class Umapper_Messages
{
    ...
    {
        // make sure that correct locale is loaded
        if (null == Wordpress_Umapper::$pluginLocale) {
            Wordpress_Umapper::setupTextdomain();
        }
        ...
        $this->messages = array(
            'API_KEY_REQUIRED'    => array(
                'txt' => '<strong>' . __('UMapper requires API key.', 'umapper') . '</strong> '
                        . sprintf(__('You must <a href="%1$s">enter your umapper.com API key</a> for plugin to work.', 'umapper'), 
                                     'plugins.php?page=umapper-config'),
                'layout' => array()
            ),
            ...
            'REQUIREMENTS_FAILED' => array(
                'txt' => '<strong>' . __('UMapper Requirements Failed.', 'umapper') . '</strong> '
                        . sprintf(__('Plugin requires PHP %1$s+ and WordPress %2$s+ to work correctly!', 'umapper'), Wordpress_Umapper::$requirements['php'], Wordpress_Umapper::$requirements['wp']),
                'layout' => array()
            ),
            'API_KEY_RESET'       => array(
