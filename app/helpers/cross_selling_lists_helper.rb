module CrossSellingListsHelper
  ...
  def status_help_text
    ...
    if @cross_selling_list.locked? then
      ...
      product_visibility = "Items on lists whose status is '#{@cross_selling_list.translate_status("Published")}' will appear in #{visibility_target} product catalog "
    end
    
    raw(current_status + product_visibility + visibility_condition)
  end
