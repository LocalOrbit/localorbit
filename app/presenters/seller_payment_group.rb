class SellerPaymentGroup
  ...
  def self.for_user(user)
    scope = Order.select('orders.*, products.organization_id as organization_id').joins(:items => :product).
    ...
    orders_by_seller_id = scope.group_by {|order| order.organization_id }

    orders_by_seller_id.map {|org_id, orders| new(org_id, orders) }.sort_by {|s| s.name }
  end
