...
Controller =
  updateBatchConsignmentReceiptStatus: (status) ->
    switch
...
View =
  batchInvoiceProgress: -> $(".batch-consignment-receipt-progress")
  batchInvoiceProgressText: -> $(".batch-consignment-receipt-progress-text")
  batchInvoiceProgressSpinner: -> $(".batch-consignment-receipt-progress-spinner")
  batchInvoiceReady:    -> $(".batch-consignment-receipt-ready")
  batchInvoiceErrors:   -> $(".batch-consignment-receipt-errors")
  batchInvoiceDownload: -> $(".batch-consignment-receipt-download")
  batchInvoiceClose:    -> $(".batch-consignment-receipt-close")
  batchInvoiceFailed:    -> $(".batch-consignment-receipt-failed")
  batchInvoicePartialFail:    -> $(".batch-consignment-receipt-partial-fail")
  batchInvoiceSupportNotice: -> $(".batch-consignment-receipt-support-notice")
...
initPoller = ->
  ...
  if uri?
    View.init()
    Poller.uri = uri
    Poller.callback = Controller.updateBatchConsignmentReceiptStatus
    Poller.start()
  ...
  sequenceStates: (states,interval) ->
    handle = setInterval ->
      ...
      if s?
        ...
        Controller.updateBatchConsignmentReceiptStatus(s)
      else
